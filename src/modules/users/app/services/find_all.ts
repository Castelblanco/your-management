import { TUserFilterDOM, TUserOPT } from '@users/domain/entities';
...
export const buildFindAll = ({ repository, getDateFormat }: Dependencies) => {
    const service = async (filter: TUserFilterDOM, options: TUserOPT) => {
        if (filter.startTime) {
            filter.startTime = getDateFormat(+filter.startTime, 'iso');
        }
        if (filter.endTime)
            filter.endTime = getDateFormat(+filter.endTime, 'iso');
